...
class AudioEngine {
    ...
    private _ensureChannels() {
        if (!this.masterStrip) {
            ...
            // Output Chain: [Master Strip] -> [Destination]
            // REMOVED Limiter to prevent compression artifacts/pumping.
            // this.masterLimiter = new Tone.Limiter(-0.1).toDestination();
            // this.masterStrip.muteGain.connect(this.masterLimiter);

            this.masterStrip.muteGain.toDestination();
