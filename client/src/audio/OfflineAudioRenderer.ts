...
export class OfflineAudioRenderer {
  static async renderProject(project: ProjectData): Promise<Blob> {
    ...
    return this._bufferToWave(
      (offlineBuffer as unknown as Tone.ToneAudioBuffer).get() as AudioBuffer,
    )
  }
  ...
  private static _configureMixer(project: ProjectData, masterHeadroom: Tone.Volume, context: any) {
    ...
    for (let i = 1; i <= 4; i++) {
      ...
      this._setupStrip(strip, groupData, mixerStrips.master.input, context)
    }
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      this._setupStrip(strip, insertData, targetInput, context)
    }
  ...
  private static _createInstruments(
    ...
    mixerStrips: {
      master: MixerStrip
      groups: Record<string, MixerStrip>
      inserts: Record<string, MixerStrip>
    },
  ) {
    const trackInstruments = new Map<string, Tone.Sampler | Tone.PolySynth | Tone.MonoSynth>()

    for (const trackId of Object.keys(project.tracks)) {
      const trackData = project.tracks[trackId]
      if (trackData.muted) continue

      let instrument: Tone.Sampler | Tone.PolySynth | Tone.MonoSynth | undefined

      if (trackData.instrument.type === 'sampler') {
        const buffer = bufferMap.get(trackId)
        if (!buffer?.loaded) {
          console.warn(`[Offline] Buffer not loaded for track ${trackId}. Skipping.`)
          continue
        }
        ...
        if (trackData.instrument.synthType === 'monosynth') {
          instrument = new Tone.MonoSynth(params as unknown as Tone.MonoSynthOptions)
        } else {
          const synthOptions = {
      ...
      const destInput = mixerStrips.inserts[insertId]
        ? mixerStrips.inserts[insertId].input
        : mixerStrips.master.input
  ...
  private static _scheduleEvents(
    project: ProjectData,
    trackInstruments: Map<string, Tone.Sampler | Tone.PolySynth | Tone.MonoSynth>,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    transport: any,
    ...
    const trackEvents = new Map<string, ScheduledNote[]>()

    // 1. Gather all notes from the timeline
    project.timeline.clips.forEach((clip) => {
      ...
      const pattern =
        project.drumPatterns[clip.patternId] || project.melodicPatterns[clip.patternId]
      if (!pattern) return
      ...
      Object.keys(pattern.clips).forEach((trackId) => {
        ...
        let events = trackEvents.get(trackId)
        if (!events) {
          events = []
          trackEvents.set(trackId, events)
        }
    ...
    for (const [trackId, events] of trackEvents.entries()) {
      ...
      const isMono = project.tracks[trackId]?.instrument.synthType === 'monosynth'
      let lastTime = -1

      events.forEach((note) => {
        // Prevent MonoSynth overlap/simultaneous trigger crashes
        if (isMono) {
          if (note.time - lastTime < 0.001) {
            return // Skip exact overlaps
          }
          lastTime = note.time
        }
  ...
  private static async _preLoadBuffers(
    project: ProjectData,
    ...
    Object.values(project.tracks).forEach((track) => {
      if (track.instrument.type === 'sampler' && track.instrument.sampleId) {
        ...
        const p = new Promise<void>((resolve) => {
          const buff = new Tone.ToneAudioBuffer(
            url,
            () => {
            ...
            (e) => {
              console.error(`[Offline] Error loading ${track.id} from ${url}:`, e)
              resolve()
            },
          )
        })
