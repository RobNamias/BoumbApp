...
const MainLayout: React.FC<MainLayoutProps> = ({ children }) => {
  ...
  const [currentView, setCurrentView] = useState<
    "skyline" | "studio" | "synthlab" | "mixer" | "settings"
  >("studio");
  ...
  const { updateTimelineClip, addTimelineClip, createPattern } =
    useProjectStore();
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (activeData.type === "kit" && overData.type === "kit-target") {
      ...
      const drumTracks = Object.values(projectTracks).filter(
        (t) => t.type === "drums",
      );
      ...
      drumTracks.forEach((track) => {
        ...
        const samplePath =
          samples[track.name] ||
          samples[
            Object.keys(samples).find((k) => track.name.includes(k)) || ""
          ];
  ...
  return (
    <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
      ...
      <div className={styles.layoutContainer}>
        ...
        <BrowserPanel isOpen={isBrowserOpen} />
