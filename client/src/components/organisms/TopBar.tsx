...
import { useProjectStore, type ProjectData } from "../../store/projectStore";
...
const MiniKnob = ({
  ...
  >
    ...
    />
    <span
      style={{ fontSize: "9px", color: "#666", textTransform: "uppercase" }}
    >
      {label}
    </span>
  </div>
...
const TopBar: React.FC<TopBarProps> = ({ isRecording, onRecord }) => {
  ...
  const loadProjectVersion = (version: ProjectVersion) => {
    useLoadingStore.getState().setLoading(true, "Chargement du projet...");
    try {
      if (version?.data) {
        const restoredData = {
          ...version.data,
          backendId: selectedProjectId,
        ...
        setVersionList([]);
        showNotification(
          t("topbar.notifications.loaded", {
            name: restoredData.meta.title || "Project",
  ...
  const handleFileLoad = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showNotification(
        t("topbar.notifications.loaded", {
          name: secureProject.meta?.title || "Project",
          version: "File",
        }),
      );
    } catch (err) {
  ...
  const loadVersions = async (projId: string, name?: string) => {
    ...
    try {
      ...
      setVersionList(versions as ProjectVersion[]);
  ...
  return (
    <div className={styles.topBar}>
      ...
      <div className={styles.leftSection}>
        <TransportControls
          isPlaying={isPlaying}
          isPaused={!isPlaying && playingStep > 0}
          bpm={bpm}
          ...
          onBpmChange={(val) => {
            ...
            useProjectStore.setState((state) => ({
              project: {
                ...state.project,
                meta: { ...state.project.meta, bpm: val },
              },
            }));
          }}
          playMode={playMode}
        ...
        >
          <GlobalKeySelector
            ...
            onChange={(root, scale) =>
              useProjectStore.getState().setGlobalKey(root, scale)
            }
          />
        </div>
      </div>
      ...
      <div className={styles.centerSection}>
          ...
          items={[
            {
              ...
              onClick: () => {
                ...
                globalThis.open(
                  `${import.meta.env.BASE_URL}manual/${lang}/index.html`,
                  "_blank",
                );
              },
            },
        ...
        <span className={styles.projectInfo}>
          {project.meta.title}{" "}
          {project.backendId ? (
            <span className={styles.badge}>{t("topbar.cloud")}</span>
          ) : (
      ...
      <div className={styles.rightSection}>
        ...
        <div className={styles.mixControls}>
          <MiniKnob
            value={juicyVolume}
            onChange={setJuicyVolume}
            color="#FF5722"
            label={t("topbar.mix.juicy")}
          />
          <MiniKnob
            value={synthVolume}
            onChange={setSynthVolume}
            color="#2196F3"
            label={t("topbar.mix.synth")}
          />
          <div className={styles.masterControl}>
            ...
            />
            <button
              onClick={() => setMasterMute(!isMasterMuted)}
              className={styles.muteBtn}
            >
              <div
                className={`${styles.ledContainer} ${isMasterMuted ? "" : styles.active}`}
              >
      ...
      <Modal
        ...
        title={t("topbar.modals.load_title")}
      >
        {selectedProjectId ? (
          // Step 2: Version List (Drill Down)
          <div className={modalStyles.versionView}>
            <div
              className={modalStyles.versionHeader}
              style={{
                display: "flex",
                alignItems: "center",
                marginBottom: "10px",
            ...
            <div className={modalStyles.projectList}>
              {versionList.map((ver) => (
                <button
                  key={ver.id}
                  className={modalStyles.projectItem}
