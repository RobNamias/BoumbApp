...
const TimelineDebugOverlay: React.FC = () => {
  const { playMode, playingStep, isPlaying } = usePlaybackStore();
  ...
  useEffect(() => {
    ...
    const chars =
      "ﾊﾐﾋｰｳｼﾅﾓﾆｻﾜﾂｵﾘｱﾎﾃﾏｹﾒｴｶｷﾑﾕﾗｾﾈｽﾀﾇﾍｦｲｸｺﾘｱﾎﾃﾏｹﾒｴｶｷﾑﾕﾗｾﾈｽﾀﾇﾍ1234567890";
  ...
  const drumPatternName =
    project.drumPatterns[activePatterns.drums || ""]?.name || "None";
  const melodyPatternName =
    project.melodicPatterns[activePatterns.melody || ""]?.name || "None";

  // Status Indicator Logic
  const getStatusIndicator = () => {
    if (isPlaying) return <span style={{ color: "#0f0" }}>▶ PLAYING</span>;
    return (
      <span style={{ color: "#ff0", animation: "blink 1.5s infinite" }}>
        ⏸ READY
      </span>
    );
  };
      ...
      >
        ...
        >
          ...
          <div>
            ...
            <div style={{ color: playMode === "SKYLINE" ? "#aff" : "#ff0" }}>
              [{playMode}]
            </div>
          </div>
        </div>
        ...
        <div style={{ marginBottom: "8px" }}>
          ...
          <div>
            STEP : {(playingStep ?? 0).toString().padStart(3, "0")}{" "}
            {renderBar(playingStep ?? 0, 16)}
          </div>
          <div>CPU : {renderBar(engineTime * 10, 10)}</div>
        </div>

        <div
          style={{
            borderTop: "1px dashed #005500",
            paddingTop: "4px",
            marginTop: "4px",
          }}
        >
