...
// Helper functions (Moved outside component to be stable/pure)
...
const PianoRoll: React.FC<PianoRollProps> = ({ trackId, patternId }) => {
  const { playingStep, isPlaying } = usePlaybackStore()
  // --- Store Access ---
  const {
    project,
    addNote: storeAddNote,
    removeNote: storeRemoveNote,
  ...
  // Track the end of a drag to prevent accidental click (deletion)
  ...
  React.useEffect(() => {
    ...
    const handleMouseUp = (e: MouseEvent) => {
      ...
      lastDragEndTime.current = e.timeStamp
      setDragState(null)
    }
  ...
  const handleGridClick = (e: React.MouseEvent<HTMLDivElement>, noteNumber: number) => {
    ...
    const existingNoteIndex = notes.findIndex(
      (n) => n.note === noteLabel && timeToStep(n.time) === clickStep,
    )
  ...
  const handleNoteClick = (e: React.MouseEvent, note: Note) => {
    ...
    // Ignore click if it happened right after finishing a resize drag
    if (e.timeStamp - lastDragEndTime.current < 150) return
    ...
    <div data-testid="piano-roll-container" className="piano-roll">
      ...
      <div className="piano-roll__scroller" ref={scrollerRef}>
        <div
          ...
          style={{ minWidth: `${VISIBLE_STEPS * STEP_WIDTH + 60}px`, position: 'relative' }}
        >
          {/* PLAYHEAD OVERLAY */}
          ...
          <div className="piano-roll__header" ref={headerRef}>
            ...
            <div
              className="piano-roll__timeline"
              style={{ width: `${VISIBLE_STEPS * STEP_WIDTH}px` }}
            >
              {stepCols.map((step) => {
                const isBeatStart = step % 4 === 0
                return (
                  <div key={step} className="piano-roll__header-step" data-is-beat={isBeatStart}>
                    {step + 1}
                  </div>
                )
              })}
                ...
                <div key={note} className="piano-roll__row">
                  ...
                  <div
                    ...
                    onKeyDown={(e) => {
                      ...
                      if (e.key === 'Enter' || e.key === ' ') handleGridClick(e as any, note)
                    }}
                    style={{ width: `${VISIBLE_STEPS * STEP_WIDTH}px` }}
                  >
                    {rowNotes.map((n, idx) => {
                      ...
                      const isDraggingThis =
                        dragState?.note.time === n.time && dragState?.note.note === n.note
                      const displaySteps = isDraggingThis
                        ? dragState.currentDurationSteps
                        : durationToSteps(n.duration)
