...
import { useProjectStore, type Note } from "../../../store/projectStore";
...
const JuicyBox: React.FC = () => {
  ...
  // Using global playback tracker from AudioEngine via Zustand
  useProjectAudio();
  ...
  const uiTracks: Track[] = useMemo(() => {
    const drumTracks = Object.values(project.tracks).filter(
      (t) => t.type === "drums",
    );
  ...
  const handleStepInteraction = (updatedTrack: Track) => {
    ...
    updatedTrack.steps.forEach((step, index) => {
      ...
      const six = index % 4;
      const time = `${bar}:${beat}:${six}`;

      const stepObj = (
        typeof step === "number"
          ? { active: step > 0, value: step, volume: 0.8, fill: 1 }
          : step
      ) as { active: boolean; value: number; volume?: number; fill?: number };
  ...
  return (
    ...
    >
      ...
      <ModuleHeader title="JuicyBox" icon={Citrus} color="#ff9800">
        ...
        <div style={{ margin: "0 auto" }}>
          <PatternSelector
            ...
            onCreatePattern={() => {
              ...
              const letter = String.fromCodePoint(65 + nextIndex); // 0 -> A, 1 -> B
