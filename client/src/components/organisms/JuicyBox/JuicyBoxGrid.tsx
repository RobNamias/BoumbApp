...
import Sequencer, { type Step } from './Sequencer'
...
const JuicyBoxGrid: React.FC<JuicyBoxGridProps> = ({
  ...
  const handleStepChange = (trackId: string, stepIndex: number, newValue?: number) => {
    console.log('JuicyBoxGrid: handleStepChange', trackId, stepIndex)
    const track = tracks.find((t) => t.id === trackId)
    console.log('JuicyBoxGrid: track found?', !!track)
    if (!track) return
    ...
    if (typeof step === 'number') {
      step = { active: step > 0, value: step, volume: 0.8, fill: 1 }
    } else {
      step = { ...step }
      if (step.volume === undefined) step.volume = 0.8
      if (step.fill === undefined) step.fill = 1
    }
    ...
    if (viewMode === 'trigger') {
      ...
      if (newValue !== undefined) {
        ...
        if (!typedStep.active) {
          ...
          if ((typedStep.fill ?? 1) >= 0.9) typedStep.fill = 0.5
          else if ((typedStep.fill ?? 0.5) >= 0.45) typedStep.fill = 0.33
          else if ((typedStep.fill ?? 0.33) >= 0.3) typedStep.fill = 0.25
          ...
          <div className="juicy-timeline-row">
            {(() => {
              ...
              return new Array(maxSteps).fill(null).map((_, i) => (
                <div
                  key={`timeline-${i}`}
                  className={`timeline-cell ${i % 4 === 0 ? 'beat-marker' : ''}`}
                >
                  {i % 4 === 0 ? i / 4 + 1 : ''}
                </div>
              ))
