...
const BrowserPanel: React.FC<BrowserPanelProps> = ({ isOpen }) => {
  ...
  useEffect(() => {
    ...
    useLoadingStore
      .getState()
      .setLoading(true, "Chargement de la bibliothèque...");
  ...
  return (
    <div
      className={`${styles.browserPanel} ${!isOpen ? styles.collapsed : ""}`}
    >
          ...
          <div key={kitId} className={styles.kitSection}>
            ...
            >
              <div
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" || e.key === " ") toggleKit(kitId);
                }}
              >
                {expandedKits[kitId] ? (
                  <ChevronDown size={14} />
                ...
                <Package size={14} color="#ff5722" />{" "}
                {/* JuicyBox Orange for Kits */}
