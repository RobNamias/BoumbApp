...
const DragInput: React.FC<DragInputProps> = ({
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      const now = Date.now()
      if (now - lastUpdateRef.current > 80 && newValue !== value) {
        // 80ms -> ~12Hz dispatch rate
        onChange(newValue)
  ...
  return (
    <div
      ...
      aria-label={label}
    >
      <span className="drag-input__value">{isDragging ? localValue : value}</span>
      {!hideLabel && <span className="drag-input__label">{label}</span>}
      {/* Hidden input for accessibility/forms */}
      <input type="hidden" value={isDragging ? localValue : value} name={label} readOnly />
