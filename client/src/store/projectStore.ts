...
export interface InstrumentConfig {
  ...
  sampleId?: string;
  // Synth
  synthType?: "fmsynth" | "polysynth" | "monosynth";
  synthParams?: {
    ...
    // Nouveaux paramètres MonoSynth (Acid)
    portamento?: number;
...
const createMixerChannel = (
  id: string,
  name: string,
  output: string = "master",
): MixerChannel => ({
  id,
  name,
  volume: 0.8,
...
const DEFAULT_PROJECT: ProjectData = {
  ...
  tracks: {
    ...
    "track-kick": createDefaultTrack(
      "track-kick",
      "Kick",
      "drums",
      "sampler",
      "insert-1",
      import.meta.env.BASE_URL + "samples/kits/808/Kick.ogg",
    ),
    "track-snare": createDefaultTrack(
      "track-snare",
      "Snare",
      "drums",
      "sampler",
      "insert-2",
      import.meta.env.BASE_URL + "samples/kits/808/Snare.ogg",
    ),
    "track-hhc": createDefaultTrack(
      "track-hhc",
      "HiHat C",
      "drums",
      "sampler",
      "insert-3",
      import.meta.env.BASE_URL + "samples/kits/808/ClosedHat.ogg",
    ),
    "track-hho": createDefaultTrack(
      "track-hho",
      "HiHat O",
      "drums",
      "sampler",
      "insert-4",
      import.meta.env.BASE_URL + "samples/kits/808/OpenHat.ogg",
    ),
    "track-perc1": createDefaultTrack(
      "track-perc1",
      "Crash",
      "drums",
      "sampler",
      "insert-5",
      import.meta.env.BASE_URL + "samples/kits/808/Crash.ogg",
    ),

    // --- 2 MELODY TRACKS ---
    "track-lead": createDefaultTrack(
      "track-lead",
      "Lead Synth",
      "melody",
      "synth",
...
export const useProjectStore = create<ProjectState>((set, get) => ({
  ...
  createPattern: (type, name) => {
    ...
    if (currentCount >= MVP_LIMITS.PATTERNS) {
      console.warn(
        `MVP Limit Reached: Cannot create more than ${MVP_LIMITS.PATTERNS} patterns of type ${type}.`,
      );
      return "";
    }
  ...
  updateMixerChannel: (channelId, channelUpdates) => {
    set((state) => {
      ...
      // Sync Audio
      if (channelUpdates.volume !== undefined)
        audioInstance.setChannelVolume(channelId, channelUpdates.volume);
      if (channelUpdates.muted !== undefined)
        audioInstance.setChannelMute(channelId, channelUpdates.muted);
      if (channelUpdates.solo !== undefined)
        audioInstance.setChannelSolo(channelId, channelUpdates.solo);
      if (channelUpdates.pan !== undefined)
        audioInstance.setChannelPan(channelId, channelUpdates.pan);
      if (channelUpdates.output !== undefined)
        audioInstance.setChannelOutput(channelId, channelUpdates.output);
  ...
  addChannelEffect: (channelId, effect) => {
    set((state) => {
      ...
      if (Object.keys(initialParams).length === 0) {
        ...
        if (type === "reverb")
          initialParams = { decay: 1.5, preDelay: 0.01, mix: 0.5 };
        else if (type === "delay")
          initialParams = { time: 0.25, feedback: 0.5, mix: 0.5 };
        else if (type === "distortion")
          initialParams = { amount: 0.4, mix: 0.5 };
        else if (type === "chorus")
          initialParams = {
            frequency: 4,
            delayTime: 2.5,
  ...
  updateChannelEffect: (channelId, effectId, updates) => {
    set((state) => {
      ...
      if (updates.params && !updates.type && updates.enabled === undefined) {
        const effectIndex = newChannel.effects.findIndex(
          (fx) => fx.id === effectId,
        );
        if (effectIndex !== -1) {
          audioInstance.updateChannelEffect(
            channelId,
            effectIndex,
            updates.params,
          );
  ...
  addNote: (patternId, trackId, note) => {
    set((state) => {
      ...
      if (
        (isDrum && state.activePatterns.drums === patternId) ||
        (!isDrum && state.activePatterns.melody === patternId)
      ) {
  ...
  removeNote: (patternId, trackId, note) => {
    set((state) => {
      ...
      clips[trackId] = trackNotes.filter(
        (n) => !(n.time === note.time && n.note === note.note),
      );
  ...
  fetchAllProjects: async () => {
    try {
      ...
      // @ts-expect-error - ProjectState might not fully match the summary list initially
  ...
  loadProjectVersion: async (versionId: string) => {
    try {
      ...
      const version = await projectService.getLatestVersion(versionId) as { data?: ProjectData } | null;
