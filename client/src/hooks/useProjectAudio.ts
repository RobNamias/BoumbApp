...
export const useProjectAudio = () => {
  ...
  const { playMode } = usePlaybackStore() // 'PATTERN' | 'SONG' (Legacy naming: 'SKYLINE'?)

  // We only want to resync CLIPS if the track STRUCURE changes (added/removed/type changed), 
  // NOT when a track's volume, pan, or synth filters change!
  const tracksStructureSig = Object.values(project.tracks).map(t => `${t.id}:${t.type}`).sort((a,b)=>a.localeCompare(b)).join(',')
  const tracksStructure = useMemo(() => {
    return Object.values(project.tracks).map(t => ({ id: t.id, type: t.type }))
    ...
    if (playMode === 'PATTERN') {
      ...
      tracksStructure
        ...
        .forEach((track) => {
          ...
          if (
            drumPid &&
            project.drumPatterns[drumPid] &&
            project.drumPatterns[drumPid].clips[track.id]
          ) {
      ...
      tracksStructure
        ...
        .forEach((track) => {
          let notes: Note[] = []
          if (activePatterns.melody) {
            const melodyPattern = project.melodicPatterns[activePatterns.melody]
            if (melodyPattern?.clips) {
              notes = melodyPattern.clips[track.id] || []
            }
          }
      ...
      // SONG MODE
      const songEvents: Record<string, Note[]> = {}

      // 1. Initialize Event Lists for all tracks
      tracksStructure.forEach((t) => {
        songEvents[t.id] = []
      })
      ...
      project.timeline.clips.forEach((clip) => {
        ...
        const pattern = isDrum
          ? project.drumPatterns[clip.patternId]
          : project.melodicPatterns[clip.patternId]
        if (!pattern) {
          console.warn(
            `[useProjectAudio] Pattern not found for clip ${clip.id} (PatternID: ${clip.patternId})`,
          )
          return
        }
      ...
      Object.keys(songEvents).forEach((trackId) => {
        ...
        if (events.length > 0) {
          console.log(
            `[useProjectAudio] Track ${trackId}: Scheduling ${events.length} events. Range: ${events[0].time} to ${events[events.length - 1].time}`,
          )
        }
